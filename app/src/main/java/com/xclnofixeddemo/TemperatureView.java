...
public class TemperatureView extends ChartView {
    ...
    public void refresh(List<PointD> linePoint) {
        dataSeriesTemp.setLineDataSet(linePoint);
...
//定义数据轴标签显示格式
    ...
    {
        dataSeriesTemp = new SplineData("", null,
                Color.parseColor("#81c784"));
        dataSeriesTemp.setLabelVisible(false);
        dataSeriesTemp.setDotStyle(XEnum.DotStyle.DOT);
        dataSeriesTemp.setDotRadius(6);
        dataSeriesTemp.getDotLabelPaint().setColor(Color.parseColor("#81c784"));
        dataSeriesTemp.setLabel(null);
        dataSeriesTemp.getLabelOptions().getBox().getBackgroundPaint().setColor(Color.WHITE);

        dataSeriesTemp.getLabelOptions().setLabelBoxStyle(XEnum.LabelBoxStyle.CAPROUNDRECT);

        chartData.add(dataSeriesTemp);
